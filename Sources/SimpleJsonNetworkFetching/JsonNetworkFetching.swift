...
/// Status Code from `200 - 299`
public let defaultStatusCodeSet = Set(200 ... 299)

/// Using `URLRequest.CachePolicy.useProtocolCachePolicy`
...
public typealias NetworkFetchingData = FetchingAndDownload & URLSessionDataDelegate & NSObject // URLSessionDownloadDelegate
public typealias NetworkFetchingDownload = FetchingAndDownload & URLSessionDownloadDelegate & NSObject // URLSessionDownloadDelegate

public protocol FetchingAndDownload: JsonNetworkFetching {
    associatedtype Download: DownloadTask
    var downloadTasks: [Download] { get set }
    func download<T: Method>(url: URL, httpMethod: T, statusCodeSet: Set<Int>, cachePolicy: URLRequest.CachePolicy, timeoutInterval: TimeInterval, completionHandler: ((Result<Download.DownloadedData, Error>) -> Void)?, progressHandler: ((Download.AnyNumber) -> Void)?) -> Result<Download, NetworkFetchingError>
...
public protocol JsonNetworkFetching: AnyObject {
    ...
    func request<T: Method>(url: URL, httpMethod: T, statusCodeSet: Set<Int>, cachePolicy: URLRequest.CachePolicy, timeoutInterval: TimeInterval, completionHandler: @escaping (Result<Data, NetworkFetchingError>) -> Void)
    
    /// The method can return `String` from the response's data.
    func request<T: Method>(url: URL, httpMethod: T, statusCodeSet: Set<Int>, cachePolicy: URLRequest.CachePolicy, timeoutInterval: TimeInterval, completionHandler: @escaping (Result<String, NetworkFetchingError>) -> Void)
    
    /// This method returns Decodable Model
    ///
    /// The returned Model can be generated by model generator which conforming to Decodable.
    func request<Output: Decodable, T: Method>(url: URL, httpMethod: T, statusCodeSet: Set<Int>, cachePolicy: URLRequest.CachePolicy, timeoutInterval: TimeInterval, completionHandler: @escaping (Result<Output, NetworkFetchingError>) -> Void)
    static func createRequest<T: Method>(url: URL, httpMethod: T, cachePolicy: URLRequest.CachePolicy, timeoutInterval: TimeInterval) -> Result<URLRequest, NetworkFetchingError>
...
public extension JsonNetworkFetching {
    ...
    func request<Output: Decodable, T: Method>(
        url: URL,
        httpMethod: T,
        statusCodeSet: Set<Int> = defaultStatusCodeSet,
        cachePolicy: URLRequest.CachePolicy = defaultURLRequestCachePolicy,
...
extension FetchingAndDownload where Download.SessionTask == URLSessionDataTask {
    ...
    public func download<T: Method>(url: URL, httpMethod: T, statusCodeSet: Set<Int> = defaultStatusCodeSet, cachePolicy: URLRequest.CachePolicy = defaultURLRequestCachePolicy, timeoutInterval: TimeInterval = defaultURLRequestTimeoutInterval, completionHandler: ((Result<Download.DownloadedData, Error>) -> Void)?, progressHandler: ((Download.AnyNumber) -> Void)?) -> Result<Download, NetworkFetchingError> {
        ...
        case let .success(request):
            ...
            let downloadTask = Download(task: task, progressHandler: progressHandler, completionHandler: completionHandler)
            downloadTask.resume()
...
public extension JsonNetworkFetching {
    ...
    static func get(headers: [String: String] = [:]) -> some Method {
        return MethodGet.get(headers: headers)
    }
    #else
    /// when wanting to use `GET`, use this static method instead of case `get` of HTTPMethod.
    ...
    /// when wanting to use `GET`, use this static method instead of case `get` of HTTPMethod.
    static func get(headers: [String: String] = [:]) -> MethodGet {
        return MethodGet.get(headers: headers)
    }
    ...
    static func createRequest<T: Method>(url: URL,
        ...
        if httpMethod.method == T.post, let body = httpMethod.body {
            do {
...
public extension FetchingAndDownload where Self: URLSessionDelegate {
    init(urlConfig: URLSessionConfiguration) {
        self.init()
...
public extension FetchingAndDownload where Self: URLSessionDataDelegate, Download.SessionTask == URLSessionDataTask, Download: DataBuffer, Download.AnyNumber == Double, Download.DownloadedData == Data {
    ...
    func urlSession(_: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            if let e = error {
                self.downloadTasks[index].completionHandler?(.failure(e))
            } else {
...
public extension FetchingAndDownload where Self: URLSessionDownloadDelegate, Download.DownloadedData == URL, Download.AnyNumber == Double {
