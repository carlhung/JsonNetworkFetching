...
public protocol JsonNetworkFetching: AnyObject {
    ...
    func download<T: Encodable>(url: URL, httpMethod: HTTPMethod<T>, statusCodeSet: Set<Int>, cachePolicy: URLRequest.CachePolicy, timeoutInterval: TimeInterval) -> Result<DownloadTask, NetworkFetchingError>
...
public extension JsonNetworkFetching {
    ...
    func download<T: Encodable>(url: URL, httpMethod: HTTPMethod<T>, statusCodeSet: Set<Int> = defaultStatusCodeSet, cachePolicy: URLRequest.CachePolicy = defaultURLRequestCachePolicy, timeoutInterval: TimeInterval = defaultURLRequestTimeoutInterval) -> Result<DownloadTask, NetworkFetchingError> {
        switch Self.createRequest(url: url, httpMethod: httpMethod, cachePolicy: cachePolicy, timeoutInterval: timeoutInterval) {
...
public extension JsonNetworkFetching where Self: URLSessionDataDelegate {
    ...
    init(urlConfig: URLSessionConfiguration) {
        self.init()
        self.session = URLSession.init(configuration: urlConfig, delegate: self, delegateQueue: nil)
    }
